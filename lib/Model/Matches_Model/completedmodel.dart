...
//
//     final completedapi = completedapiFromJson(jsonString);

import 'dart:convert';

Completedapi completedapiFromJson(String str) => Completedapi.fromJson(json.decode(str));
...
class Matches {
  ...
  factory Matches.fromJson(Map<String, dynamic> json) => Matches(
    completed: List<Ted>.from(json["COMPLETED"].map((x) => Ted.fromJson(x))),
    notstarted: List<Ted>.from(json["NOTSTARTED"].map((x) => Ted.fromJson(x))),
